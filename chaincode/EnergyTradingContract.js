...
class EnergyTradingContract extends Contract{
  ...
  async initLedger(ctx) {
      ...
      if (!this.isAdmin(identity)) {
        throw new Error(`Network can only be initialized by administrators`);
      }
  ...
  async createAsset(ctx, participantId, id, producer, energyType, units) {
      ...
      if (!this.isProducer(identity) && !this.isAdmin(identity)) {
        throw new Error(`Assets can only be created by producer or administrators`);
      }
